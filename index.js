...
axios.get(`https://Sever-Uptime.abcdz1238.repl.co?add=${UrlRepl}`)
		console.log(chalk.bold.hex(randomColor()).bold("[ FCA-Shadow ]", global.fca.languages.uptime + UrlRepl));
	}
	catch {
		console.log(chalk.bold.hex(randomColor()).bold("[ FCA-Shadow ]", global.fca.languages.erroUptime));
  }
}
...
console.log(chalk.bold.hex(randomColor()).bold("[ FCA ]", global.fca.languages.login));
        return utils
            ...
            .then(function(res) {
                ...
                if (headers.location.indexOf('https://www.facebook.com/checkpoint/') > -1) {
                    console.log(chalk.bold.hex(randomColor()).bold("[ FCA ]", global.fca.languages.errorFa));
                                            ...
                                            utils
                                                ...
                                                .then(res => {
                                                    try {
                                                        ...
                                                        clearInterval(checkVerified);
                                                        console.log(chalk.bold.hex(randomColor()).bold("[ FCA ]", global.fca.languages.okweb));
                                                        if (callback === prCallback) {
                                                            callback = function(err, api) {
                                                                if (err) return prReject(err);
                                                                return prResolve(api);
...
require('dotenv').config({ path: './../.env' });
                ...
                var ans = makeid(49)
                    process.env["FBKEY"] = ans;
                        fs.writeFile('./../.env', `FBKEY=${ans}`, function (err) {
                            if (err) {
                            console.log(chalk.bold.hex(randomColor()).bold("[ FCA-Shadow ]", global.fca.languages.errorEnv));
                    }
                else console.log(chalk.bold.hex(randomColor()).bold("[ FCA-Shadow ]", global.fca.languages.env))
        }); 
    }
    catch (e) {
        console.log(e);
        console.log(chalk.bold.hex(randomColor()).bold("[ FCA-Shadow ]", global.fca.languages.errorPassRandom));
    }
}
    ...
    if (process.env['FBKEY']) {
        try {
            ...
            if (appState.includes('[')) {
                console.log(chalk.bold.hex(randomColor()).bold("[ FCA-Shadow ]", global.fca.languages.appstateDec));
            } else {
                try {
                    appState = JSON.parse(appState);
                    var StateCrypt = require('./StateCrypt');
                    appState = StateCrypt.decryptState(appState, process.env['FBKEY']);
                    console.log(chalk.bold.hex(randomColor()).bold("[ FCA-Shadow ]", global.fca.languages.appstate));
                }
                catch (e) {
                    console.log(chalk.bold.hex(randomColor()).bold("[ FCA-Shadow ]", global.fca.languages.appstateError));
                }
            }
        }
        catch (e) {
    ...
    try {
        ...
        try {
            ...
            return console.log(chalk.bold.hex(randomColor()).bold("[ FCA-Shadow ]", global.fca.languages.appstateError))
        }
    }
      }
    try { 
    ...
    mainPromise
        .then(function() {
            console.log(chalk.bold.hex(randomColor()).bold("[ FCA ]", global.fca.languages.oklogin));
          
                    console.log(chalk.bold.hex(randomColor()).bold("[ FCA ]", `${global.fca.languages.checkVersion}` +"Premium"+ ' !'));      
